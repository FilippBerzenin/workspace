 ...
 * @version 3.0
 */
public class tankFinal extends JPanel {
...
//		moveToQuadrant(4, 5);
	...
	void clean() throws Exception {
		for (int i = 0;i<((battleField.length*battleField.length)); i++) {
			int [][] targetCoordinate =  targetCoordinate ();
			System.out.println(Arrays.toString(targetCoordinate)+"in void clean");
//			int v = targetCoordinate[0];
//			int h = targetCoordinate[1];
//			moveToQuadrant(h, v);
		}
	}
	
	int[][] targetCoordinate  () throws Exception {
		int [][] targetCoordinate = new int [2][battleField.length*battleField.length];
		int h = 0;
		int v = 0;
		for (int i = 0; i<battleField.length*battleField.length; i++) {
		for (v = 0;v<battleField.length-1;v++) {
			for (h = 0; h<battleField[v].length-1; h++) {
				System.out.println(v+" "+h);
				if (battleField[v][h].equals("B")) {
					targetCoordinate[0][i] = h;
					}
			} 	if (battleField[v][h].equals("B")) {
				System.out.println(battleField[v][h]+"target");
				targetCoordinate[1][i] = v+1;
		}
	}
		}
		System.out.println(Arrays.toString(targetCoordinate[0]));
		System.out.println(Arrays.toString(targetCoordinate[1]));
		return targetCoordinate;
}
	
	int vertikal () {
		int vertikal = (tankY/64)-1;
		return vertikal; 
	}
	
	int horizont () {
		int horizont = (tankX/64)-1;
		return horizont; 
	}

	void firefoTarget()  throws Exception  {	
		if (tankDirection == 1) {
			for(int i = 0; i<battleField.length; i++) {
				if (scanTargetTop()==false) {
				fire();
				}
				else tankDirection = 2;
			}
			}
		...
		if (tankDirection == 2) {
			for(int i = 0; i<battleField.length; i++) {
				if (scanTargetBottom()==false) {
				fire();
				}
				else tankDirection = 3;
			}
			}
		if (tankDirection == 3) {
			for(int i = 0; i<battleField.length; i++) {
				if (scanTargetLeft()==false) {
				fire();
				}
				else tankDirection = 4;
			}
			}
		if (tankDirection == 4) {
			for(int i = 0; i<battleField.length; i++) {
				if (scanTargetRight()==false) {
				fire();
				}
				else break;
			}
	...
	boolean scanTargetTop() throws Exception {
...
//				System.out.println(indexVertikal + " " + indexHorizontal);
	...
	boolean scanTargetBottom() throws Exception {
		int indexVertikal = tankY/64;
		int indexHorizontal = tankX/64;
		if (tankDirection == 2) {
...
//				System.out.println(indexVertikal + " " + indexHorizontal);
	...
	boolean scanTargetLeft() throws Exception {
		int indexVertikal = tankY/64;
		int indexHorizontal = tankX/64;
		if (tankDirection == 3) {
...
//				System.out.println(indexVertikal + " " + indexHorizontal);
	...
	boolean scanTargetRight() throws Exception {
		int indexVertikal = tankY/64;
		int indexHorizontal = tankX/64;
		if (tankDirection == 4) {
...
//				System.out.println(indexVertikal + " " + indexHorizontal);
	...
	public static void main(String[] args) throws Exception {
		tankFinal bf = new tankFinal();
		bf.runTheGame();
	}

	public tankFinal() throws Exception {
