...
public class frame_4_12 {
	...
	static String pupilSurname = "Ohrimenko";   // Ввести фамилию и имя ученика, по которому необходима статистика. 
	static String pupilName = "Maks";
		...
		errorMain(); // проверка на ошибки.
		
		printStatisticBadRatingForOne(indexName(name), cartulary(name, subject, rating, indexName(name))); // статистика по плохим оценка интересующего ученика.
	
//		printStatisticForOne(indexName(name),  cartulary(name, subject, rating, indexName(name))); // статистика по общим результатам интересующего ученика.
		
//		printStatisticForAll(indexName(name), cartulary(name, subject, rating, indexName(name))); // общая статистика по группе.

		
	
		}
		
		static void errorMain() {
			if (error() == false) {
		...
		static void printStatisticBadRatingForOne (String [][] indexName, String [][][] cartulary) {
			...
			for (int k = 1; k<subject.length+1; k++) {
				System.out.println(Arrays.toString(cartulary[i][k]));
					...
					System.out.println("have "+x+" bad rating,"+" and "+(cartulary[i][k].length-1)+" lessons.");
					x = 0;
				}
		}
		...
		static boolean error () {
			if (errorArraysNull(name, subject, rating) == false || errorArraysName(name) == false ||
					errorArraysSubject (subject) == false || errorArraysRating (rating) == false
					|| errorName(name, pupilSurname, pupilName) == false) {
				return false;
			}
			return true;
		}
		
		static boolean errorName (String[][] name, String pupilSurname,  String pupilName) {
			for (int i = 0; i<name.length; i++) {
				if(name[i][0].equals(pupilSurname) && name[i][1].equals(pupilName)) {
					return true;
				}
			} 
			return false;
